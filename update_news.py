# ----------------------------------------------------
# 智財經 Finance Hub - Expanded News Data Generator
# ----------------------------------------------------
import os
import json
import datetime
...
def get_taiwan_date_str():
    ...
    weekday_str = weekdays[tw_time.weekday()]
    return tw_time.strftime(f"%Y年%m月%d日 {weekday_str}")

def generate_expanded_dataset():
    date_str = get_taiwan_date_str()

    market_cards = [
        {"symbol": "台股加權指數", "ticker": "TAIEX", "val": "23,920.80", "change": "+210.15 (+0.88%)", "isUp": True, "tag": "台股主板"},
        {"symbol": "納斯達克指數", "ticker": "NASDAQ", "val": "18,710.45", "change": "+145.20 (+0.78%)", "isUp": True, "tag": "美股科技"},
        {"symbol": "輝達 (NVIDIA)", "ticker": "NVDA", "val": "$131.20", "change": "+$2.70 (+2.10%)", "isUp": True, "tag": "AI領航"},
        {"symbol": "台積電 ADR", "ticker": "TSM", "val": "$179.50", "change": "+$2.70 (+1.53%)", "isUp": True, "tag": "晶片龍頭"},
        {"symbol": "台灣六都房價指數", "ticker": "TW-HOUSING", "val": "348.0", "change": "-0.05% (量縮盤整)", "isUp": False, "tag": "台灣房市"},
        {"symbol": "美元 / 新台幣", "ticker": "USD/TWD", "val": "32.42", "change": "-0.03 (-0.09%)", "isUp": False, "tag": "外匯"},
        {"symbol": "黃金 (Gold)", "ticker": "XAU/USD", "val": "$2,450.60", "change": "+$18.40 (+0.76%)", "isUp": True, "tag": "避險資產"},
        {"symbol": "布蘭特原油", "ticker": "BRENT", "val": "$84.30", "change": "-$0.65 (-0.77%)", "isUp": False, "tag": "能源大宗"}
    ]
    ...
    return date_str, market_cards, digest_highlights, news_items

def update_app_js():
    date_str, market_cards, digest_highlights, news_items = generate_expanded_dataset()
    
    js_content = f"""// ----------------------------------------------------
// 智財經 Finance Hub - Interactive Logic & Data Engine
// Updated Automatically at: {date_str}
// ----------------------------------------------------

const currentReportDate = "{date_str}";

// 1. Data Store: Market Overview Cards
const marketData = {json.dumps(market_cards, ensure_ascii=False, indent=2)};

// 2. Data Store: 30-Second Digest Highlights
const digestHighlights = {json.dumps(digest_highlights, ensure_ascii=False, indent=2)};

// 3. Main News Database
const newsDatabase = {json.dumps(news_items, ensure_ascii=False, indent=2)};
...
"""
    ...
    with open("/Users/mars/finance-dashboard/app.js", "w", encoding="utf-8") as f:
        f.write(js_content)
    print("Successfully updated app.js with 18 comprehensive news items!")
